...
<?php
        ...
        if (isset($_SESSION['login']) && $_SESSION['login'] == true) {
            ...
            if ((isset($_POST['deleteid'])) && ($_POST['deleteid'] != "") && ($id === $targetid)) {
                // Get target user id
                $uid2 = $_POST['deleteid'];

                // Delete from table
                $newquery1 = "DELETE FROM FriendWith WHERE uid1 = '$id' AND uid2 = '$uid2'";
                $newquery2 = "DELETE FROM FriendWith WHERE uid1 = '$uid2' AND uid2 = '$id'";
            ...
            $query = "SELECT ID, firstName, lastName, city FROM Users 
                        WHERE ID <> '$targetid' AND ID IN (SELECT uid2 FROM FriendWith WHERE uid1 = '$targetid')";
            $result = $mysqli->query($query);
            $numrows = $result->num_rows;

            // Get target user's first name
            $query = "SELECT firstName FROM Users WHERE ID = '$targetid'";
            $newresult = $mysqli->query($query);
            $newrow = mysqli_fetch_array($newresult);

            // Print number of friends
            echo "<h2>" .$newrow[0]. "'s Friends <span class=\"label label-danger\">$numrows</span></h2><br>";
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<div class='col-md-" .$colwidth. "'>";
                        echo "<div class='card' style='width: 100%;'>";
                            ...
                            // Only delete friend if user views his/her own list
                            if ($id === $targetid) {
                                echo "<form method='post' action='friendlist.php?targetid=$targetid'>";
                                echo "<input type='hidden' name='deleteid' value='". $row['ID'] ."'>";
                                echo "<button type='submit' class='btn btn-info'>Unfriend</button></td>";
                                echo "</form>";
                            }
                            echo "</div>";
                        echo "</div>";
