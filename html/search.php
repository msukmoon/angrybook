...
<?php
    ...
    if (isset($_SESSION['login']) && $_SESSION['login'] == true) {
        // If user sends the form /Search results
        if ((isset($_POST['search'])) && ($_POST['search'] != "")) {
            // Get current user id
            $search = $_POST['search'];
            echo("<script>location.href='search.php?search=$search';</script>");                 
        ...
        // Search title
        echo "<h2>Search Results <span class=\"label label-danger\">$numrows</span></h2><br>";
